...
error SEIZNFTContract_MaxSupplyReached();
error SEIZNFTContract_ValueNotEqualPrice();
error SEIZNFTContract_WrongAvenueForThisTransaction();

/// @custom:security-contact derrickj.jarrell@gmail.com
contract SEIZNFTContract is 
    ERC721,
    ERC721Enumerable, 
    ERC721URIStorage, 
    ERC721Burnable, 
    ...
    ReentrancyGuard 
{
    ...
    receive() external payable {
        revert SEIZNFTContract_WrongAvenueForThisTransaction();
    }
    ...
    function mintTo(
        ...
        if (tokenId >= i_max_tokens) {
            revert SEIZNFTContract_MaxSupplyReached();
    }
    //make sure there is the right amount of money
        if (msg.value != i_mint_price) {
        revert SEIZNFTContract_ValueNotEqualPrice();
    }
    _tokenIdCounter++;
